...
method_fn <- function(grp_data_culled, sample_pop_id_list, msi_real, c, m_colnames, n, n_boot, iter_num, samp_size, bootstrap_size, boots, method) {
  ...
  trend.dev <- jaccard(as.numeric(msi_final), as.numeric(msi_real), testNA = FALSE)
  ...
  trend.dev.list <- lapply(msi_final_samp_list, function(i) {if(anyNA(i)) {NA} else {jaccard(as.numeric(i), as.numeric(msi_real), testNA = FALSE)}})
