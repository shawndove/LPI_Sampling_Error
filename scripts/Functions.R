...
ci_fn <- function(index, c, m_colnames, boots=100, n=NA, weights=NA, lambda=FALSE) {
    ...
    for (grp in grp_ids) {
      ...
      if (!(tail(m_colnames, 1) %in% colnames(grp_specdata))) {
        ...
        if (!is.na(weights)) {
          ...
          if (length(weights) != nrow(grp_specdata1.4)) {
          ...
          for (i in 1:length(weights)) {
            ...
            temp <- grp_specdata1.4[i,] * weights[i]
          ...
          grp_specdata1.4 <- do.call(rbind, w_grpdata)
      ...
      for (i in 1:boots) {
        ...
        for (j in 1:ncol(grp_specdata1.4)) {
          ...
          if(length(temp)==0) {
            ...
            temp2 <- sample(temp, length(temp), replace=TRUE)
        ...
        if (is.na(weights)) {
          ...
          # take the mean of the natural log of the species lambdas
          grp_samp_temp1.4 <- colSums(grp_samp_temp, na.rm=TRUE)
