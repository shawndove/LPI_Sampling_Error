...
all_fn <- function(popvar, 
  ...
  msi_ci_real <- ci_fn(grp_real, c, m_colnames, grouplevel=2)
...
foreach(i = 1:100) %dopar% {  # loop for parallel processing
  all_fn(popvar = gr_sd_vec_a, # variance in mean growth rate
         ...
         mlength = j_choice_a, # mean length of time series 
         numobs = k_choice_a, # mean number of observations in each time series
