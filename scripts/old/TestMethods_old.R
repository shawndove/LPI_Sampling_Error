...
method_fn <- function(grp_data_culled, sample_pop_id_list, msi_real, grp_real, c, m_colnames, n, n_boot, iter_num, samp_size, bootstrap_size, method) {
  ...
  if (method=="lr" | method=="my") {
    ...
    msi_ci_full <- ci_fn(msi_full, c, m_colnames, n=n, lambda=lambda, grouplevel=2)
    ...
    # create final confidence intervals
    msi_ci_full <- ci_fn(grp_lambda, c, m_colnames, lambda=TRUE, grouplevel=2)
    saveRDS(msi_ci_full, file=paste("TestData/", iter_num, "/saved_synth_", iter_num, "_msi_ci_", method, ".RData", sep=""))
    ...
    msi_ci_full <- ci_fn(grp_full, c, m_colnames, lambda=TRUE, grouplevel=2)
    saveRDS(msi_ci_full, file=paste("TestData/", iter_num, "/saved_synth_", iter_num, "_msi_ci_", method, ".RData", sep=""))
  ...
  samp_list <- lapply(sample_pop_id_list, function(i) {
    ...
    if (method=="lr" | method=="my") {
      ...
      msi_ci <- ci_fn(msi_samp, c, m_colnames, n=n, lambda=lambda, grouplevel=2)
      ...
      # create msi confidence intervals
      msi_ci <- ci_fn(grp_lambda_samp, c, m_colnames, lambda=TRUE, grouplevel=2)
      ...
      # create msi confidence intervals
      msi_ci <- ci_fn(grp_samp, c, m_colnames, lambda=TRUE, grouplevel=2)
