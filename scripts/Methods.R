...
method_fn <- function(grp_data_culled, sample_pop_id_list, msi_real, grp_real, c, m_colnames, n, n_boot, iter_num, samp_size, bootstrap_size, method) {
  ...
  trend.dev <- devdist(msi_final, msi_real)
  ...
  trend.dev.grp.list <- lapply(1:length(grp_final), function(i) {devdist(grp_final[[i]][,1:c], grp_real[[i]][,1:c])})
  ...
  trend.dev.list <- lapply(msi_final_samp_list, function(i) {devdist(i, msi_real)})
  ...
  trend.grp.dev.list <- lapply(grp_final_samp_list, function(i) {lapply(1:length(i), function(j) {devdist(i[[j]][,1:c], grp_real[[j]][,1:c])})})
